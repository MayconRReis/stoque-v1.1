...
export const supabaseService = {
 ...
 if (!isSupabaseConfigured) {
 const items = localStorageHelper.get('inventory').filter((r: any) => r.id !== id);
 localStorageHelper.save('inventory', items);
 return;
 }
 const { error } = await supabase
 .from('inventory')
 ...
 if (!isSupabaseConfigured) {
 const currentSlots = localStorageHelper.get('warehouse_slots');
 const newSlots = currentSlots.map((s: any) => {
 const update = slots.find(u => u.id === s.id);
 return update ? { ...s, ...update } : s;
 });
 const missingSlots = slots.filter(s => !currentSlots.some((cs: any) => cs.id === s.id));
 localStorageHelper.save('warehouse_slots', [...newSlots, ...missingSlots]);
 return;
 }
 const { error } = await supabase
 .from('warehouse_slots')
 ...
 if (!isSupabaseConfigured) {
 const shipments = localStorageHelper.get('shipments').filter((s: any) => s.id !== shipmentId);
 localStorageHelper.save('shipments', shipments);
 
 const items = await this.getInventoryItemsByShipmentId(shipmentId);
 const itemIds = items.map(i => i.id);
 const all = localStorageHelper.get('inventory');
 const updated = all.map((row: any) => {
 if (itemIds.includes(row.id)) {
 const newRow = { ...row };
 newRow.inspections = newRow.inspections.map((insp: any) => {
 ...
 localStorageHelper.save('inventory', updated);
 return;
 }
 ...
 if (grouped[row.id]) {
 const newRow = { ...row };
 grouped[row.id].forEach((idx: number) => {
 if (newRow.inspections && newRow.inspections[idx]) {
 if (shipmentId === null) {
 delete newRow.inspections[idx].shipmentId;
 delete newRow.inspections[idx].shipment_id;
 } else {
 newRow.inspections[idx].shipmentId = shipmentId;
 }
 }
 });
 return newRow;
 ...
 if (!isSupabaseConfigured) {
 const items = localStorageHelper.get('rotative_stock').filter((r: any) => r.id !== id);
 localStorageHelper.save('rotative_stock', items);
 return;
 }
 const { error } = await supabase
 .from('rotative_stock')
