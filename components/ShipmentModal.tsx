      ...
      >
        <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-slate-200/20 dark:bg-slate-800/20">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-fuchsia-600 rounded-xl flex items-center justify-center text-slate-900 dark:text-white shadow-xl">
              <Truck className="w-5 h-5" />
            </div>
            <div>
              <h3 className="text-lg font-black text-slate-900 dark:text-white uppercase italic tracking-tight">Enviar para Carregamento</h3>
              <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest">{selectedCount} pallets selecionados</p>
            </div>
          </div>
          <button onClick={onClose} className="w-8 h-8 rounded-lg bg-slate-50 dark:bg-slate-950 text-slate-500 hover:text-slate-900 dark:hover:text-white flex items-center justify-center border border-slate-200 dark:border-slate-800 transition-colors">
            <X className="w-4 h-4" />
          </button>
        </div>
          ...
          {mode === 'choice' && (
            <div className="grid grid-cols-1 gap-4">
              <button 
                ...
                className="group p-6 bg-slate-50/50 dark:bg-slate-950/50 border border-slate-200 dark:border-slate-800 hover:border-fuchsia-500/50 rounded-3xl text-left transition-all hover:bg-slate-200/30 dark:hover:bg-slate-800/30"
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="text-slate-900 dark:text-white font-bold uppercase text-sm">Criar Novo Carregamento</h4>
              ...
              <button 
                ...
                className="group p-6 bg-slate-50/50 dark:bg-slate-950/50 border border-slate-200 dark:border-slate-800 hover:border-pink-500/50 rounded-3xl text-left transition-all hover:bg-slate-200/30 dark:hover:bg-slate-800/30 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="text-slate-900 dark:text-white font-bold uppercase text-sm">Adicionar a Existente</h4>
            ...
            <div className="space-y-6">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button 
                      ...
                      className={`py-3 rounded-xl font-bold text-[10px] uppercase tracking-widest border transition-all ${type === ShipmentType.THIRD_PARTY ? 'bg-fuchsia-600 border-fuchsia-400 text-slate-900 dark:text-white shadow-lg shadow-fuchsia-900/20' : 'bg-slate-50 dark:bg-slate-950 border-slate-200 dark:border-slate-800 text-slate-500 hover:border-slate-700'}`}
                    >
                      Terceirista
                    </button>
                    <button 
                      onClick={() => setType(ShipmentType.OWN)}
                      className={`py-3 rounded-xl font-bold text-[10px] uppercase tracking-widest border transition-all ${type === ShipmentType.OWN ? 'bg-pink-600 border-pink-400 text-slate-900 dark:text-white shadow-lg shadow-pink-900/20' : 'bg-slate-50 dark:bg-slate-950 border-slate-200 dark:border-slate-800 text-slate-500 hover:border-slate-700'}`}
                    >
                      Próprio
                    </button>
                  </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input 
                      ...
                      className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl pl-12 pr-4 py-3 text-slate-900 dark:text-white font-bold text-sm focus:border-fuchsia-600 outline-none transition-all"
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button 
                  ...
                  className="flex-[2] py-4 bg-fuchsia-600 hover:bg-fuchsia-500 disabled:bg-slate-200 dark:bg-slate-800 text-slate-900 dark:text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl shadow-fuchsia-900/40 transition-all flex items-center justify-center gap-2"
                >
                  {isProcessing ? (
                ...
                <div className="max-h-[300px] overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                  {openShipments.map(shipment => (
                    <button 
                      ...
                      className={`w-full p-4 rounded-2xl border text-left transition-all ${selectedShipmentId === shipment.id ? 'bg-fuchsia-600/10 border-fuchsia-500 shadow-lg' : 'bg-slate-50 dark:bg-slate-950 border-slate-200 dark:border-slate-800 hover:border-slate-700'}`}
                    >
                      <div className="flex justify-between items-start">
                        <div>
                          <p className="text-[10px] font-black text-slate-900 dark:text-white font-mono uppercase mb-1">{shipment.id}</p>
