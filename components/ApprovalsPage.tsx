...
const ApprovalsPage: React.FC<ApprovalsPageProps> = ({ currentUser }) => {
 ...
 <div className="px-4 py-2 bg-slate-900 rounded-xl border border-slate-800 flex flex-col items-center">
 ...
 <div className="bg-slate-900 border-2 border-dashed border-slate-800 rounded-[32px] py-12 text-center">
...
const RequestCard = ({ 
 ...
 return (
 <div className={`bg-slate-900 backdrop-blur-md rounded-[2.5rem] border transition-all duration-300 overflow-hidden ${isExpanded ? 'border-amber-500/50 ring-1 ring-amber-500/20' : 'border-slate-800 hover:border-slate-700 '}`}>
 ...
 <div className="mt-4 p-4 bg-slate-950 rounded-2xl border border-amber-500/10">
 ...
 className="w-full bg-slate-900 border border-slate-800 rounded-2xl px-4 py-3 text-white font-medium text-sm focus:border-purple-500 outline-none transition-all resize-none"
 rows={2}
 />
 </div>
 ...
 className="flex-[2] py-4 bg-green-600 hover:bg-green-500 text-zinc-50 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all flex items-center justify-center gap-2 shadow-xl shadow-green-900/20 active:scale-95 disabled:opacity-50"
 >
 {isProcessing ? (
...
const DataPreview = ({ data, baseColor, isChanged, comparison }: { data: any, baseColor: string, isChanged?: boolean, comparison?: any }) => {
 ...
 return (
 <div className={`p-5 rounded-3xl bg-slate-950 border ${isChanged ? 'border-purple-500/20' : 'border-slate-800 '} space-y-4`}>
...
const HistoryCard = ({ request }: { request: InventoryEditRequest }) => {
 ...
 return (
 <div className="bg-slate-900 rounded-[2rem] border border-slate-800 p-6 flex flex-col h-full hover:border-slate-700 transition-all">
