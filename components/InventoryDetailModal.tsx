...
export const InventoryDetailModal: React.FC<InventoryDetailModalProps> = ({ isOpen, onClose, row, inspection, palletIdx, onUnconsolidate, isUnconsolidating }) => {
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-50/90 dark:bg-slate-950/90 backdrop-blur-2xl p-4 overflow-y-auto">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[48px] shadow-[0_32px_64px_-12px_rgba(0,0,0,0.8)] w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95 duration-500 my-auto">
        ...
        <div className="bg-slate-200/30 dark:bg-slate-800/30 p-8 flex justify-between items-center border-b border-slate-200/50 dark:border-slate-800/50">
          <div className="flex items-center gap-4">
             <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-lg transform -rotate-3 ${
                ...
                <Icon className="text-slate-900 dark:text-white w-6 h-6" />
             </div>
             <div>
                <h3 className="font-black text-xl italic uppercase tracking-tighter text-slate-900 dark:text-white">Detalhes do Item</h3>
             </div>
          </div>
        ...
        <div className="p-8 space-y-8">
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-slate-50/50 dark:bg-slate-950/50 p-4 rounded-2xl border border-slate-200/50 dark:border-slate-800/50">
               <p className="text-[9px] text-slate-600 font-black uppercase mb-1">ID Técnico</p>
               <p className="text-sm font-black text-slate-900 dark:text-white font-mono">{row.loadingId}</p>
            </div>
            <div className="bg-slate-50/50 dark:bg-slate-950/50 p-4 rounded-2xl border border-slate-200/50 dark:border-slate-800/50">
               <p className="text-[9px] text-slate-600 font-black uppercase mb-1">Data de Entrada</p>
               <p className="text-sm font-black text-slate-900 dark:text-white font-mono">{row.date}</p>
            </div>
          </div>

          <div className="space-y-4">
            <p className="text-[10px] text-slate-600 font-black uppercase tracking-widest italic">Descrição do Produto</p>
            <h4 className="text-lg text-slate-900 dark:text-white font-black uppercase leading-tight tracking-tight">{row.description}</h4>
          ...
          {isSupplies && (
            <div className="bg-slate-200/20 dark:bg-slate-800/20 p-6 rounded-[32px] border border-slate-200/40 dark:border-slate-800/40 space-y-6">
              <div className="flex justify-between items-center mb-0">
                  <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest">Conferência Técnica</p>
                  <span className={`bg-slate-50 dark:bg-slate-950 px-4 py-1.5 rounded-full text-[10px] font-black border border-slate-200 dark:border-slate-800 uppercase italic ${inspection.assignedSlot === 'AGUARDANDO' ? 'text-amber-500' : 'text-blue-400'}`}>
                    {inspection.assignedSlot === 'AGUARDANDO' ? 'AG. VAGA' : `Vaga ${inspection.assignedSlot}`}
                  </span>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                <div><p className="text-[8px] font-black text-slate-600 uppercase mb-2">Caixas</p><p className={`text-lg font-black ${inspection.boxes > 0 ? 'text-slate-900 dark:text-white' : 'text-slate-800'}`}>{inspection.boxes}</p></div>
                <div><p className="text-[8px] font-black text-slate-600 uppercase mb-2">Frascos</p><p className={`text-lg font-black ${inspection.bottles > 0 ? 'text-slate-900 dark:text-white' : 'text-slate-800'}`}>{inspection.bottles}</p></div>
                <div><p className="text-[8px] font-black text-slate-600 uppercase mb-2">Berços</p><p className={`text-lg font-black ${inspection.cradles > 0 ? 'text-slate-900 dark:text-white' : 'text-slate-800'}`}>{inspection.cradles}</p></div>
                <div><p className="text-[8px] font-black text-slate-600 uppercase mb-2">Tampas</p><p className={`text-lg font-black ${inspection.caps > 0 ? 'text-slate-900 dark:text-white' : 'text-slate-800'}`}>{inspection.caps}</p></div>
              </div>
                ...
                <div className="pt-6 border-t border-slate-200/50 dark:border-slate-800/50 space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2 justify-center">
                    {inspection.others.map((other, idx) => (
                      <div key={idx} className="bg-slate-50 dark:bg-slate-950 px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-800 flex items-center gap-2">
                        <span className="text-[9px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-tight">{other.name}</span>
          ...
          {!isSupplies && (
            <div className="bg-slate-200/20 dark:bg-slate-800/20 p-6 rounded-[32px] border border-slate-200/40 dark:border-slate-800/40 space-y-4">
               <div className="flex justify-between items-center">
                  <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest">Localização</p>
                  <span className={`bg-slate-50 dark:bg-slate-950 px-6 py-2.5 rounded-full text-xs font-black border border-slate-200 dark:border-slate-800 uppercase italic tracking-widest ${inspection.assignedSlot === 'AGUARDANDO' ? 'text-amber-500' : 'text-blue-400'}`}>
                    {inspection.assignedSlot === 'AGUARDANDO' ? 'AG. VAGA' : `Vaga ${inspection.assignedSlot}`}
                  </span>
               </div>
          ...
          {row.is_group && children.length > 0 && (
            <div className="bg-slate-200/20 dark:bg-slate-800/20 p-6 rounded-[32px] border border-slate-200/40 dark:border-slate-800/40 space-y-4">
              ...
              <div className="max-h-40 overflow-y-auto space-y-2 pr-2">
                {children.map(child => (
                  <div key={child.id} className="bg-slate-50 dark:bg-slate-950 p-3 rounded-xl border border-slate-200 dark:border-slate-800 flex justify-between items-center">
                    <span className="text-xs font-bold text-slate-700 dark:text-slate-300 font-mono">{child.loadingId}</span>
          ...
          <button onClick={onClose} className="w-full py-5 bg-blue-600 hover:bg-blue-500 text-slate-900 dark:text-white rounded-2xl font-black text-xs uppercase tracking-[0.3em] shadow-xl shadow-blue-900/40 transition-all active:scale-95">Fechar Detalhes</button>
        </div>
      </div>
    </div>
  );
