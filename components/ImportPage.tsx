...
export const ImportPage: React.FC<ImportPageProps> = ({ onProcess }) => {
 ...
 <div className="bg-slate-900 backdrop-blur-xl p-8 md:p-16 rounded-[2.5rem] border border-slate-800 shadow-3xl text-center relative overflow-hidden">
 ...
 exit={{ opacity: 0, y: 20 }}
 className="bg-slate-900 backdrop-blur-xl rounded-[2.5rem] border border-slate-800 shadow-3xl overflow-hidden"
 >
 <div className="p-6 md:p-8 border-b border-slate-800 flex flex-col sm:flex-row justify-between items-center bg-slate-800 gap-4">
 <div>
 ...
 className="w-full sm:w-auto px-6 py-3 bg-blue-600 hover:bg-blue-500 disabled:bg-slate-800 text-zinc-50 rounded-xl font-bold text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg shadow-blue-900/20 transition-all"
 >
 {isProcessing ? (
 ...
 <th className="p-5 text-[9px] font-bold text-slate-500 uppercase tracking-widest border-b border-slate-800 ">Status</th>
 <th className="p-5 text-[9px] font-bold text-slate-500 uppercase tracking-widest border-b border-slate-800 ">OP / Nome</th>
 <th className="p-5 text-[9px] font-bold text-slate-500 uppercase tracking-widest border-b border-slate-800 ">Lote</th>
 <th className="p-5 text-[9px] font-bold text-slate-500 uppercase tracking-widest border-b border-slate-800 ">Qtd / Tipo</th>
 </tr>
 </thead>
 <tbody className="divide-y divide-slate-800/30">
 {items.map((item, idx) => (
 <tr key={idx} className={`hover:bg-slate-800 transition-colors ${!item.selected ? 'opacity-40' : ''}`}>
 <td className="p-5">
 <button 
 onClick={() => {
 ...
 className={`w-5 h-5 rounded-lg border-2 flex items-center justify-center transition-all ${item.selected ? 'bg-blue-600 border-blue-400 text-zinc-50 ' : 'bg-slate-950 border-slate-800 text-transparent'}`}
 >
