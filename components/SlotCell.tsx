...
const SlotCell: React.FC<SlotCellProps> = ({ slot, onClick, getContainerColor }) => {
 ...
 className={`aspect-square rounded-2xl border flex flex-col items-center justify-center p-2 transition-all duration-200 group relative cursor-pointer shadow-sm hover:shadow-md ${
 ...
 >
 <span className="text-[10px] md:text-xs font-bold text-slate-500 mb-1.5">{slot.id.split('.').slice(1).join('.')}</span>
 {ContentIcon && (
 <ContentIcon className={`w-5 h-5 md:w-6 md:h-6 ${
