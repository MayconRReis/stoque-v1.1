...
const Logo: React.FC<{ size?: 'sm' | 'md' }> = ({ size = 'md' }) => {
 ...
 <Warehouse className={`${isSm ? 'w-5 h-5' : 'w-6 h-6'} text-zinc-50 `} />
 </div>
 <div>
...
const NavItem = memo(({ tab, icon: Icon, label, badge, isActive, onNavigate, activeTab }: { tab: string, icon: React.ElementType, label: string, badge?: number, isActive: boolean, onNavigate: (tab: any) => void, activeTab: string }) => (
 ...
 className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all relative group ${isActive ? 'bg-blue-600 text-zinc-50 shadow-lg shadow-blue-900/20' : 'hover:bg-slate-800 text-slate-400 hover:text-slate-200 '}`}
 >
 <Icon className={`w-4 h-4 ${isActive ? 'text-zinc-50 ' : 'text-slate-500 group-hover:text-slate-300 '}`} />
...
const SubTabs = ({ tabs, activeSubTab, onSubTabChange }: { tabs: { id: string, label: string }[], activeSubTab: string, onSubTabChange: (id: any) => void }) => (
 <div className="flex gap-2 p-1 bg-slate-900 backdrop-blur-md rounded-2xl border border-slate-800 mb-8 overflow-x-auto no-scrollbar flex-shrink-0">
 {tabs.map(tab => (
 <button
 key={tab.id}
 ...
 activeSubTab === tab.id 
 ? 'bg-blue-600 text-zinc-50 shadow-lg shadow-blue-900/20' 
...
const App: React.FC = () => {
 ...
 return (
 <div className="bg-slate-900 p-5 md:p-8 rounded-[2.5rem] border border-slate-800 shadow-2xl overflow-hidden mb-6">
 ...
 <div className="bg-slate-950 px-4 py-2 rounded-xl border border-slate-800 flex items-center gap-6">
 ...
 slot.status === SlotContent.EMPTY ? 'bg-slate-950 border-slate-800 hover:border-slate-700 ' : 
 ...
 {slot.occupiedBy && (
 <div className="absolute inset-0 opacity-0 group-hover:opacity-100 bg-slate-900 backdrop-blur-sm rounded-xl flex items-center justify-center z-10 transition-opacity border border-slate-700 p-1">
 ...
 className="w-full px-6 py-4 bg-indigo-600 hover:bg-indigo-500 text-zinc-50 rounded-2xl font-bold text-xs uppercase tracking-widest flex items-center justify-center gap-3 transition-all"
 >
 ...
 className="w-full px-6 py-4 bg-blue-600 hover:bg-blue-500 text-zinc-50 rounded-2xl font-bold text-xs uppercase tracking-widest flex items-center justify-center gap-3 transition-all"
 >
 ...
 {isSidebarOpen && (
 <div 
 className="fixed inset-0 bg-slate-950 backdrop-blur-sm z-40 lg:hidden animate-in fade-in duration-300"
 ...
 {/* Sidebar Navigation */}
 {!isPublicView && (
 <aside className={`fixed lg:sticky top-0 left-0 h-screen w-72 bg-slate-900 backdrop-blur-xl border-r border-slate-800 shadow-2xl z-50 transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} flex-shrink-0 flex flex-col`}>
 ...
 className="w-7 h-7 rounded-lg bg-red-500/10 text-red-500 border border-red-500/20 flex items-center justify-center hover:bg-red-500 hover:text-zinc-50 transition-all shadow-lg"
 ...
 <header className="bg-slate-950 backdrop-blur-xl border-b border-slate-900 h-16 px-4 md:px-10 flex justify-between items-center sticky top-0 z-40 shrink-0">
 <div className="flex items-center gap-3 md:gap-4">
 {!isPublicView && (
 <button 
 onClick={() => setIsSidebarOpen(true)}
 className="lg:hidden w-10 h-10 bg-slate-900 border border-slate-800 rounded-2xl flex items-center justify-center text-slate-400 hover:text-white transition-all active:scale-95 shadow-lg"
 >
 ...
 className="p-2 bg-slate-900 text-slate-400 hover:text-white rounded-xl border border-slate-800 transition-all shadow-lg"
 title={isLightMode ? 'Alternar para Tema Escuro' : 'Alternar para Tema Claro'}
 >
 ...
 className="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-zinc-50 rounded-xl font-bold text-[9px] uppercase tracking-widest transition-all shadow-lg shadow-blue-900/20"
 >
 Acessar App
 </button>
 )}
 <div className="flex items-center justify-center w-10 h-6 bg-slate-900 rounded-full border border-slate-800 ">
 ...
 (type as any).isSub ? 'ml-4 bg-slate-950 ' : ''
 } ${
 ...
 <button 
 onClick={() => setIsShipmentModalOpen(true)}
 className="w-full md:w-auto px-5 py-3 bg-purple-600 hover:bg-purple-500 text-zinc-50 rounded-xl font-bold text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg shadow-purple-900/20 animate-in zoom-in duration-200"
 >
 ...
 <div className="bg-slate-900 p-6 rounded-[2rem] border border-slate-800 shadow-xl space-y-3 relative overflow-hidden">
 ...
 <div className="fixed inset-0 bg-slate-950 backdrop-blur-sm flex items-center justify-center z-[110] p-4">
 <motion.div 
 ...
 <div key={i} className="bg-slate-950 p-3 rounded-xl border border-slate-800 text-[10px] font-bold text-slate-300 ">
 {item}
 </div>
 ))}
 </div>
 ...
 <button 
 onClick={handleResyncSlots}
 className="px-8 py-3 bg-indigo-600 hover:bg-indigo-500 text-zinc-50 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all shadow-lg shadow-indigo-600/20 flex items-center gap-2"
 >
 ...
 {deleteContext && (
 <div className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-950 backdrop-blur-md p-4">
 ...
 <div className="flex gap-4 pt-4"><button onClick={() => setDeleteContext(null)} className="flex-1 py-3 md:py-3.5 bg-slate-800 text-slate-400 font-black text-[9px] md:text-[10px] uppercase rounded-2xl transition-all">Cancelar</button><button onClick={confirmDelete} className="flex-1 py-3 md:py-3.5 bg-red-600 text-zinc-50 font-black text-[9px] md:text-[10px] uppercase rounded-2xl shadow-lg transition-all active:scale-95">Remover</button></div>
 </div>
 </div>
 )}
 ...
 <button onClick={confirmMatrixSend} className="flex-1 py-3 md:py-3.5 bg-blue-600 text-zinc-50 font-black text-[9px] md:text-[10px] uppercase rounded-2xl shadow-lg shadow-blue-900/40 transition-all active:scale-95">Sim, Enviar</button>
 </div>
 </div>
 </div>
 )}
 ...
 {isLogoutConfirmOpen && (
 <div className="fixed inset-0 z-[150] flex items-center justify-center bg-slate-950 backdrop-blur-xl p-4">
 ...
 className="flex-1 py-4 bg-red-600 text-zinc-50 font-black text-[10px] uppercase tracking-widest rounded-2xl shadow-lg shadow-red-900/40 transition-all hover:bg-red-500 active:scale-95"
 >
 Sair
 </button>
 </div>
